...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="w-64 bg-[#0a121e] border-r border-slate-800 flex flex-col h-screen fixed left-0 top-0">
      <div className="p-8">
      ...
      <div className="mt-auto p-6 border-t border-slate-800">
        <div className="bg-[#1e293b]/30 p-4 rounded-xl">
          <p className="text-xs text-slate-500 uppercase tracking-widest mb-2">Premium Member</p>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center font-bold text-xs">JD</div>
            <div>
              <p className="text-sm font-semibold">Jane Doe</p>
              <p className="text-[10px] text-slate-400">Executive Plan</p>
            </div>
          </div>
        </div>
      </div>
